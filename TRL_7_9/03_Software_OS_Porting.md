...
Настоящий документ представляет собой углубленное архитектурно-техническое исследование и **Дорожную карту (Roadmap)** по трансферу, декуплингу, глубокому форкингу, системной адаптации и сертификации китайских базовых Open-Source технологий ([openEuler](../openEuler.md), [OpenHarmony](../OpenHarmony.md), [openGauss](../openGauss.md), [OceanBase](../OceanBase.md)) для критической информационной инфраструктуры (КИИ) РФ. 

Основной фокус направлен на преодоление базовых научно-технологических барьеров (**TRL 1–6**): разрешение аппаратных аномалий архитектуры [Эльбрус e2k](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) (VLIW), портирование JIT/AOT-компиляторов, устранение коллизий в [Linux Security Modules](../LSM.md), адаптацию криптографических стеков TEE/HUKS под стандарты ГОСТ и переработку низкоуровневых механизмов консенсуса СУБД под гетерогенные кластеры.
{% endhint %}
...
Лицензия **Mulan Permissive Software License v2 (Mulan PSL v2)**, под которой распространяются [openEuler](../openEuler.md) и [openGauss](../openGauss.md), содержит ключевые риски:
...
1. **Удаление вендорских вызовов (Vendor Lock-in Stripping):**
   ...
   * Извлечение патчей ядра Linux, специфичных для процессоров [Kunpeng 920](../Kunpeng%20920.md) (`hisilicon` SoC drivers), и их изолирование в подсистему поддержки плагинов architecture-specific.
...
#### 2.1. Глубокий инжиниринг под архитектуру [Эльбрус e2k](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) (VLIW)

Архитектура [Эльбрус e2k](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) с явным параллелизмом команд (VLIW) кардинально отличается от Out-of-Order систем (x86/ARM). Прямое компилирование кода, рассчитанного на Strong/Weak Memory Ordering, приводит к системным сбоям и фатальной потере производительности.
...
* **Проблема:** В ядре [openEuler](../openEuler.md) и СУБД [openGauss](../openGauss.md) активно используются Lock-Free структуры на базе атомиков C++11 / GCC built-ins (`__atomic_load_n`, `__atomic_compare_exchange_n`). На [Эльбрус e2k](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) слабый порядок памяти (Weak Memory Ordering) приводит к переупорядочиванию чтения/записи компилятором `LCC` и процессором.
* **Техническое решение:** 
...
* **Техническое решение:** 
  ...
  2. Переработка `ArkCompiler` (JS/TS Engine в [OpenHarmony](../OpenHarmony.md)): Отключение фазы JIT-компиляции (`ark_js_runtime`) и перевод выполнения скриптов исключительно в режим **AOT Compilation + Interpreter PGO Profile**. Внедрение генератора C++ IR-представления на базе оптимизатора `LCC`.

```
[eBPF Bytecode] ──> [AOT C-Translator] ──> [LCC Compiler (-O4)] ──> [Kernel Dynamic Module (.ko)]
```

#### 2.2. Портирование на [RISC-V](../RISC-V.md) (RV64GCB / RV64GCH)
...
* **Доработка под отечественные СнК ([Syntacore](../Syntacore.md) SCR7, [YADRO](../YADRO.md) C300/C500):**
  * Внедрение поддержки расширений векторной обработки **RVV 1.0** (*Vector Extension*) в алгоритмы криптографии ядра и СУБД.
  * Добавление поддержки расширений манипуляции битами **Zba/Zbb/Zbc/Zbs** в инструкционную обвязку компилятора GCC/Clang для ускорения хеширования в [openGauss](../openGauss.md).
...
#### 3.1. Разрешение коллизий в [Linux Security Modules](../LSM.md)

{% hint style="danger" %}
**Архитектурный конфликт LSM Hooks**
Попытка совместить штатный модуль безопасности openEuler (**Euler Security Module / SELinux**) с отечественными [КСЗ](../%D0%9A%D0%A1%D0%97.md) (Комплексы Средств Защиты: [Astra Linux Parsec](../Astra%20Linux%20Parsec.md), [Secret Net](../Secret%20Net.md), [Kaspersky Security System](../Kaspersky%20Security%20System.md)) приводит к взаимоисключающим блокировкам системных вызовов (`EPERM`), разрушению контекстов безопасности Inode (`xattr`) и неизбежному `Kernel Panic`.
{% endhint %}
...
В ядре Linux 5.10/6.6 [openEuler](../openEuler.md) реализуется строго изолированный порядок вызова хоков безопасности с переопределением структуры `security_hook_heads`:
...
#### 3.2. Интеграция криптографии ГОСТ в [OpenHarmony](../OpenHarmony.md) (HUKS & TEE)

Стек безопасности [OpenHarmony](../OpenHarmony.md) опирается на подсистему **HUKS (Harmony Universal KeyStore)** и доверенную среду **TEE (Trusted Execution Environment)**, использующие по умолчанию алгоритмы AES, RSA, ECC, SHA-256.
...
1. **Замена криптографического ядра HUKS:**
   ...
   * Переработка абстракции `huks_core_cipher.c`: перенаправление вызовов генерации ключей и шифрования на ГОСТ-сопроцессоры или программный модуль [КриптоПро](../%D0%9A%D1%80%D0%B8%D0%BF%D1%82%D0%BE%D0%9F%D1%80%D0%BE.md) / [OpenSSL Engine GOST](../OpenSSL%20Engine%20GOST.md).
...
* **Динамический анализ и фаззинг (DAST):**
  * Покрытие системных вызовов ядра ОС непрерывным фаззинг-тестированием с помощью `Syzkaller` с аппаратной виртуализацией на процессорах [Эльбрус-16С](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81-16%D0%A1.md) и [Байкал-С](../%D0%91%D0%B0%D0%B9%D0%BA%D0%B0%D0%BB-%D0%A1.md).
  * Проведение фаззинга сетевого стека СУБД [openGauss](../openGauss.md) и [OceanBase](../OceanBase.md) с помощью `AFL++` / `libFuzzer` на предмет устойчивости к Denial of Service (DoS) атакам.
...
#### 4.1. Глубокая адаптация [openGauss](../openGauss.md)
[openGauss](../openGauss.md) оптимизирована под архитектуру процессоров Huawei Kunpeng 920 (архитектура NUMA с 64+ ядрами на сокет). Она использует алгоритмы сжатия и блокировок, завязаные на специфичные ARM64 инструкции (`LDADD`, `CAS`).
...
* **Устранение жесткой NUMA-привязки:**
  ...
  * Адаптация алгоритмов планирования потоков (Thread Pooling) под процессоры [Эльбрус-16С](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81-16%D0%A1.md) (учитывая топологию межпроцессорного соединения "Канал" и объединение ядер в кластеры).
...
#### 4.2. Реинжиниринг движка консенсуса [OceanBase](../OceanBase.md)
[OceanBase](../OceanBase.md) использует архитектуру LSM-Tree и распределенный Paxos-консенсус.
...
2. **Тестовые фермы (Hardware Matrix):**
   * Физические стенды под управлением контроллеров удаленного управления (IPMI/BMC), укомплектованные платами на базе [Эльбрус-8С/16С](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81-8%D0%A1/16%D0%A1.md), [Байкал-М/С](../%D0%91%D0%B0%D0%B9%D0%BA%D0%B0%D0%BB-%D0%9C/%D0%A1.md), [Syntacore](../Syntacore.md) RISC-V FPGA/SoC.
...
| :--- | :--- | :---: | :---: | :--- | :--- |
| **[openEuler](../openEuler.md)** | Базовая Серверная / Cloud ОС | **TRL 5** | **TRL 8** | Конфликты LSM, барьеры памяти e2k | Глубокий форк, LSM-stacking под КСЗ, AOT для eBPF |
| **[OpenHarmony](../OpenHarmony.md)** | Мобильная / Edge / IoT ОС | **TRL 4** | **TRL 7** | Графический стек GPU, HDF, HUKS | Замена HUKS на ГОСТ, интеграция OP-TEE, RISC-V MCU |
| **[openGauss](../openGauss.md)** | Enterprise OLTP СУБД | **TRL 5** | **TRL 8** | NUMA-зависимость Kunpeng, uStore lock-free | Адаптация MOT под Эльбрус-16С, ГОСТ-TLS репликация |
| **[OceanBase](../OceanBase.md)** | Распределённая NewSQL СУБД | **TRL 3** | **TRL 6** | Закрытый Enterprise Paxos engine | Форк CE-версии, создание открытого модуля консенсуса |
...
3. Аппаратный стек: Серверный кластер на базе **[Байкал-С](../%D0%91%D0%B0%D0%B9%D0%BA%D0%B0%D0%BB-%D0%A1.md)** (ARM64) и периферийные контроллеры на базе **[Syntacore](../Syntacore.md) SCR5** (RISC-V).
...
* **Центральный серверный кластер (ARM64 / Байкал-С):**
  * **ОС:** Форк [openEuler](../openEuler.md) (Linux Kernel 6.6, сборка ARM64).
  * **СУБД:** Форк [openGauss](../openGauss.md) (Кластер Shared-Nothing с синхронной репликацией).
...
* **Периферийные контроллеры (RISC-V / Syntacore SCR5):**
  * **ОС:** Форк [OpenHarmony](../OpenHarmony.md) (LiteOS kernel / RTOS profile).
...
* [openEuler: Архитектура, сборка и методы портирования](../openEuler.md)
* [OpenHarmony: Стек HDF, ArkCompiler и замена HUKS](../OpenHarmony.md)
* [openGauss: Движки uStore, MOT и оптимизация NUMA](../openGauss.md)
* [OceanBase: Paxos Консенсус и LSM-Tree Архитектура](../OceanBase.md)
* [Архитектура Эльбрус VLIW: Память, Компиляция LCC и eBPF](../%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md)
* [RISC-V в РФ: Перспективы экосистемы YADRO и Syntacore](../RISC-V.md)
* [Linux Security Modules: LSM Stacking и разрешение коллизий](../LSM.md)
* [Требования ФСТЭК по Уровням Доверия (УД 1-4)](../%D0%A4%D0%A1%D0%A2%D0%AD%D0%9A%20%D0%A3%D0%94.md)
* [Mulan PSL v2: Юридический анализ и патентный декуплинг](../Mulan%20PSL%20v2.md)
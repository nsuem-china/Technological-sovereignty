...
Настоящий документ представляет собой углубленное архитектурно-техническое исследование и **Дорожную карту (Roadmap)** по трансферу, декуплингу, глубокому форкингу, системной адаптации и сертификации китайских базовых Open-Source технологий ([openEuler](../lib-1/openEuler.md), [OpenHarmony](../lib-1/OpenHarmony.md), [openGauss](../lib-1/openGauss.md), [OceanBase](../lib-1/OceanBase.md)) для критической информационной инфраструктуры (КИИ) РФ. 

| Основной фокус направлен на преодоление базовых научно-технологических барьеров (**TRL 1–6**): разрешение аппаратных аномалий архитектуры [Эльбрус e2k](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) (VLIW), портирование JIT/AOT-компиляторов, устранение коллизий в [Linux Security Modules](../lib-1/LSM.md), адаптацию криптографических стеков TEE/HUKS под стандарты ГОСТ и переработку низкоуровневых механизмов консенсуса СУБД под гетерогенные кластеры. |
| --- | --- |
...
Лицензия **Mulan Permissive Software License v2 (Mulan PSL v2)**, под которой распространяются [openEuler](../lib-1/openEuler.md) и [openGauss](../lib-1/openGauss.md), содержит ключевые риски:
...
| • Юрисдикция: КНР (Mulan PSL) • Зависимость от Huawei Cloud • Проприетарный iSula / Telemetry | • Изолированный репозиторий (ТК 22) • Замена на локальные сервисы PKI • Модульная сборка на базе Apache 2.0 |
...
1. **Удаление вендорских вызовов (Vendor Lock-in Stripping):**
   ...
   * Извлечение патчей ядра Linux, специфичных для процессоров [Kunpeng 920](../lib-1/Kunpeng%20920.md) (`hisilicon` SoC drivers), и их изолирование в подсистему поддержки плагинов architecture-specific.
...
│
     ┌───────────────────────────────────┼───────────────────────────────────┐
     ▼                                   ▼                                   ▼
[ARM64 (Baikal-C / Kunpeng)](../lib-1/Kunpeng.md)    [RISC-V (RV64GCB / Syntacore)](../lib-1/Syntacore.md)    [Elbrus VLIW (e2k v5/v6)](../lib-1/v6.md)
...
#### 2.1. Глубокий инжиниринг под архитектуру [Эльбрус e2k](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) (VLIW)

Архитектура [Эльбрус e2k](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) с явным параллелизмом команд (VLIW) кардинально отличается от Out-of-Order систем (x86/ARM). Прямое компилирование кода, рассчитанного на Strong/Weak Memory Ordering, приводит к системным сбоям и фатальной потере производительности.
...
* **Проблема:** В ядре [openEuler](../lib-1/openEuler.md) и СУБД [openGauss](../lib-1/openGauss.md) активно используются Lock-Free структуры на базе атомиков C++11 / GCC built-ins (`__atomic_load_n`, `__atomic_compare_exchange_n`). На [Эльбрус e2k](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k.md) слабый порядок памяти (Weak Memory Ordering) приводит к переупорядочиванию чтения/записи компилятором `LCC` и процессором.
* **Техническое решение:** 
...
##### Б. Переработка [ArkCompiler](../lib-1/ArkCompiler.md) (OpenHarmony) и eBPF JIT under VLIW
* **Проблема eBPF JIT:** Ядро [openEuler](../lib-1/openEuler.md) полагается на eBPF JIT для фильтрации пакетов и мониторинга (`pcap`, `systemd-journald`, `cgroups v2`). JIT-генераторы динамически создают машинный код x86/ARM64. Под e2k динамическая генерация VLIW-широких команд (Wide Instruction Words) в рантайме крайне затруднена из-за необходимости работы тяжеловесного планировщика инструкций компилятора.
* **Техническое решение:** 
  ...
  2. Переработка `ArkCompiler` (JS/TS Engine в [OpenHarmony](../lib-1/OpenHarmony.md)): Отключение фазы JIT-компиляции (`ark_js_runtime`) и перевод выполнения скриптов исключительно в режим **AOT Compilation + Interpreter PGO Profile**. Внедрение генератора C++ IR-представления на базе оптимизатора `LCC`.

[eBPF Bytecode] ──> [AOT C-Translator] ──> [LCC Compiler (-O4)] ──> [Kernel Dynamic Module (.ko)]

#### 2.2. Портирование на [RISC-V](../lib-1/RISC-V.md) (RV64GCB / RV64GCH)
...
* **Доработка под отечественные СнК ([Syntacore](../lib-1/Syntacore.md) SCR7, [YADRO](../lib-1/YADRO.md) C300/C500):**
  * Внедрение поддержки расширений векторной обработки **RVV 1.0** (*Vector Extension*) в алгоритмы криптографии ядра и СУБД.
  * Добавление поддержки расширений манипуляции битами **Zba/Zbb/Zbc/Zbs** в инструкционную обвязку компилятора GCC/Clang для ускорения хеширования в [openGauss](../lib-1/openGauss.md).
...
| #### 3.1. Разрешение коллизий в [Linux Security Modules](../lib-1/LSM.md) |
| --- | --- |


{% hint style="danger" %}
**Архитектурный конфликт LSM Hooks**
Попытка совместить штатный модуль безопасности openEuler (**Euler Security Module / SELinux**) с отечественными [КСЗ](../lib-1/%D0%9A%D0%A1%D0%97.md) (Комплексы Средств Защиты: [Astra Linux Parsec](../lib-1/Astra%20Linux%20Parsec.md), [Secret Net](../lib-1/Secret%20Net.md), [Kaspersky Security System](../lib-1/Kaspersky%20Security%20System.md)) приводит к взаимоисключающим блокировкам системных вызовов (`EPERM`), разрушению контекстов безопасности Inode (`xattr`) и неизбежному `Kernel Panic`.
{% endhint %}
...
В ядре Linux 5.10/6.6 [openEuler](../lib-1/openEuler.md) реализуется строго изолированный порядок вызова хоков безопасности с переопределением структуры `security_hook_heads`:
...
#### 3.2. Интеграция криптографии ГОСТ в [OpenHarmony](../lib-1/OpenHarmony.md) (HUKS & TEE)

Стек безопасности [OpenHarmony](../lib-1/OpenHarmony.md) опирается на подсистему **HUKS (Harmony Universal KeyStore)** и доверенную среду **TEE (Trusted Execution Environment)**, использующие по умолчанию алгоритмы AES, RSA, ECC, SHA-256.
...
1. **Замена криптографического ядра HUKS:**
   ...
   * Переработка абстракции `huks_core_cipher.c`: перенаправление вызовов генерации ключей и шифрования на ГОСТ-сопроцессоры или программный модуль [КриптоПро](../lib-1/%D0%9A%D1%80%D0%B8%D0%BF%D1%82%D0%BE%D0%9F%D1%80%D0%BE.md) / [OpenSSL Engine GOST](../lib-1/OpenSSL%20Engine%20GOST.md).
...
* **Динамический анализ и фаззинг (DAST):**
  * Покрытие системных вызовов ядра ОС непрерывным фаззинг-тестированием с помощью `Syzkaller` с аппаратной виртуализацией на процессорах [Эльбрус-16С](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81-16%D0%A1.md) и [Байкал-С](../lib-1/%D0%91%D0%B0%D0%B9%D0%BA%D0%B0%D0%BB-%D0%A1.md).
  * Проведение фаззинга сетевого стека СУБД [openGauss](../lib-1/openGauss.md) и [OceanBase](../lib-1/OceanBase.md) с помощью `AFL++` / `libFuzzer` на предмет устойчивости к Denial of Service (DoS) атакам.
...
│
                     ┌─────────────────────────┴─────────────────────────┐
                     ▼                                                   ▼
            [openGauss (Fork)](../lib-1/openGauss%20Fork.md)                                [OceanBase](../lib-1/OceanBase.md)
...
#### 4.1. Глубокая адаптация [openGauss](../lib-1/openGauss.md)
[openGauss](../lib-1/openGauss.md) оптимизирована под архитектуру процессоров Huawei Kunpeng 920 (архитектура NUMA с 64+ ядрами на сокет). Она использует алгоритмы сжатия и блокировок, завязаные на специфичные ARM64 инструкции (`LDADD`, `CAS`).
...
* **Устранение жесткой NUMA-привязки:**
  ...
  * Адаптация алгоритмов планирования потоков (Thread Pooling) под процессоры [Эльбрус-16С](../lib-1/%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81-16%D0%A1.md) (учитывая топологию межпроцессорного соединения "Канал" и объединение ядер в кластеры).
...
#### 4.2. Реинжиниринг движка консенсуса [OceanBase](../lib-1/OceanBase.md)
[OceanBase](../lib-1/OceanBase.md) использует архитектуру LSM-Tree и распределенный Paxos-консенсус.
...
| • Мониторинг Git Commits в OpenAtom / Gitee repos. • Извлечение CVE и Security Advisories. | • Авто-разрешение конфликтов в Merge Commits. • Инъекция patches в локальный декуплированный форк. | • Стенды Эльбрус-16С • Стенды Байкал-С • Стенды RISC-V (YADRO / Syntacore) |
...
2. **Тестовые фермы (Hardware Matrix):**
   * Физические стенды под управлением контроллеров удаленного управления (IPMI/BMC), укомплектованные платами на базе [Эльбрус-8С/16С](../lib-1/16%D0%A1.md), [Байкал-М/С](../lib-1/%D0%A1.md), [Syntacore](../lib-1/Syntacore.md) RISC-V FPGA/SoC.
...
| Платформа :--- **[openEuler](../lib-1/openEuler.md)** **[OpenHarmony](../lib-1/OpenHarmony.md)** **[openGauss](../lib-1/openGauss.md)** **[OceanBase](../lib-1/OceanBase.md)** | Роль в экосистеме :--- Базовая Серверная / Cloud ОС Мобильная / Edge / IoT ОС Enterprise OLTP СУБД Распределённая NewSQL СУБД | TRL (Текущий в РФ) :---: **TRL 5** **TRL 4** **TRL 5** **TRL 3** | TRL (Целевой за 24 мес) :---: **TRL 8** **TRL 7** **TRL 8** **TRL 6** | Главный барьер :--- Конфликты LSM, барьеры памяти e2k Графический стек GPU, HDF, HUKS NUMA-зависимость Kunpeng, uStore lock-free Закрытый Enterprise Paxos engine | Стратегия устранения :--- Глубокий форк, LSM-stacking под КСЗ, AOT для eBPF Замена HUKS на ГОСТ, интеграция OP-TEE, RISC-V MCU Адаптация MOT под Эльбрус-16С, ГОСТ-TLS репликация Форк CE-версии, создание открытого модуля консенсуса |
| --- | --- | --- | --- | --- | --- |
...
3. Аппаратный стек: Серверный кластер на базе **[Байкал-С](../lib-1/%D0%91%D0%B0%D0%B9%D0%BA%D0%B0%D0%BB-%D0%A1.md)** (ARM64) и периферийные контроллеры на базе **[Syntacore](../lib-1/Syntacore.md) SCR5** (RISC-V).
...
* **Центральный серверный кластер (ARM64 / Байкал-С):**
  * **ОС:** Форк [openEuler](../lib-1/openEuler.md) (Linux Kernel 6.6, сборка ARM64).
  * **СУБД:** Форк [openGauss](../lib-1/openGauss.md) (Кластер Shared-Nothing с синхронной репликацией).
...
* **Периферийные контроллеры (RISC-V / Syntacore SCR5):**
  * **ОС:** Форк [OpenHarmony](../lib-1/OpenHarmony.md) (LiteOS kernel / RTOS profile).
...
| * [openEuler: Архитектура, сборка и методы портирования](../lib-1/openEuler%20*%20OpenHarmony%20*%20openGauss%20*%20OceanBase%20*%20%D0%AD%D0%BB%D1%8C%D0%B1%D1%80%D1%83%D1%81%20e2k%20*%20RISC-V%20*%20LSM%20*%20%D0%A4%D0%A1%D0%A2%D0%AD%D0%9A%20%D0%A3%D0%94%20*%20Mulan%20PSL%20v2.md) OpenHarmony: Стек HDF, ArkCompiler и замена HUKS]] openGauss: Движки uStore, MOT и оптимизация NUMA]] OceanBase: Paxos Консенсус и LSM-Tree Архитектура]] Архитектура Эльбрус VLIW: Память, Компиляция LCC и eBPF]] RISC-V в РФ: Перспективы экосистемы YADRO и Syntacore]] Linux Security Modules: LSM Stacking и разрешение коллизий]] Требования ФСТЭК по Уровням Доверия (УД 1-4)]] Mulan PSL v2: Юридический анализ и патентный декуплинг]] |
| --- | --- |

...
1. **Аудит сетевых контроллеров и микрокодов:**
   ...
   * Дамп и бинарный аудит прошивок **SPI Flash** с контролем целостности по алгоритму **ГОСТ Р 34.11-2012 (Стрибог-256)**.
...
3. **Криптографическая изоляция:**
   * Установка внешних аппаратных криптомодулей (**ГОСТ HSM**) на интерфейсах передачи данных между **eNodeB/gNodeB** и ядром сети (**5G Core / EPC**).
...
2. **Декомпозиция сети по спецификациям **OpenRAN**:**
   ...
   * Исполнение L2/L3 функций в среде контейнеризации (**Kubernetes**) с управлением через отечественные сервисы **MANO/NMS**.
...
| **Трансфер технологий** | Принудительный через создание совместных предприятий (Joint Ventures) | Полный отказ западных/китайских вендоров от передачи IP; необходимость **реверс-инжиниринга** |

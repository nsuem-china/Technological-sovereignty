...
> Совместные фундаментальные и прикладные исследования между Российской академией наук ([[РАН]]) и Китайской академией наук ([[CAS]]) в области квантовых технологий и открытой архитектуры [[Архитектура RISC-V|RISC-V]] представляют собой стратегический альянс с высокой степенью асимметрии. 
> 
...
Для РФ [[Квантовые вычисления]] (дорожная карта Росатома) и квантовые коммуникации (дорожная карта РЖД), а также переход на [[Архитектура RISC-V|RISC-V]] (разработки Yadro/KNS Group, Syntacore, Миландр) — это вопросы выживания критической информационной инфраструктуры (КИИ) в условиях санкций.
...
* **CAS** обладает неограниченным финансовым ресурсом, доступом к современному технологическому оборудованию (cleanrooms класса ISO 3/4), развитой экосистемой [[EDA-системы|EDA-инструментов]] и возможностями быстрого прототипирования (Tape-out) на мощностях [[SMIC]].
...
* **Сотрудничество:** ИФВД РАН (Троицк) — синтез сверхчистых алмазов методом HPHT с контролируемой концентрацией азотных вакансий и изотопным обогащением по углероду-12 ($^{12}\text{C} > 99.99\%$), что минимизирует спиновый шум. CAS (Институт полупроводников) — интеграция алмазных микрокристаллов с [[Кремниевая фотоника|кремниевой фотоникой]] (waveguides, кольцевые резонаторы) и КМОП-схемами считывания на одном чипе.
...
* **Безопасность (Security Extensions):** Разработка аппаратных анклавов (типа ARM TrustZone), устойчивых к атакам по сторонним каналам (Side-channel attacks, Spectre/Meltdown). РАН (ИСП РАН) обеспечивает методы [[Формальная верификация|формальной верификации]] RTL-кода (с использованием систем Coq/Kami) на отсутствие аппаратных закладок и логических ошибок. CAS (Institute of Software) интегрирует ядра в открытую программную экосистему (OpenEuler, RISC-V Software Ecosystem (RISE)).
...
> [!warning] Экономический риск для РФ: «Ловушка локализации»
...
> [!question] Кейс 1: Защита интеллектуальной собственности при Tape-out
...
> [!question] Кейс 2: Преодоление криогенного барьера

...
{% hint style="info" %}
**Executive Summary**
Процесс замещения западных экосистем автоматизации ([Siemens TIA Portal/Simatic](../lib-1/Siemens_TIA_Portal.md), [Schneider Electric EcoStruxure/Modicon](../lib-1/Schneider_EcoStruxure.md)) распределился по двум параллельным трекам:
1. **Промышленный заказ (TRL 7–9):** Экстренное внедрение готовых китайских ПЛК и PAC-контроллеров на базе коммерческих архитектур ARM/x86 и проприетарной среды [CODESYS Control Runtime](../lib-1/CODESYS_Dependency.md#runtime-engine).
2. **Фундаментальная суверенизация (TRL 1–6):** Разработка независимых вычислительных ядер (RISC-V, LoongArch), систем жесткого реального времени с формальной верификацией кода и открытых компиляторов стандарта [IEC 61131-3 / IEC 61499](../lib-1/IEC_61499_Standard.md#distributed-runtime).
...
| **Level 2 (PLC / PAC Control)** | Simatic S7-1500, Schneider Modicon M580 | Inovance AM600/AC800, SUPCON ECS-700, Hollysys LK/K-series | [RISC-V Lockstep Cores](../lib-1/RISC_V_Industrial_Cores.md#lockstep-architecture) + Beremiz Runtime (TRL 3–5) | Нестабильность цикла вычислений FPU при высокой плотности I/O; зависимость от зарубежных компиляторов. |
| **Level 1 (Fieldbus & I/O)** | PROFINET IRT, EtherNet/IP (CIP Safety) | [EtherCAT](../lib-1/EtherCAT_Protocol.md#architecture), Modbus TCP, Inovance GL10/GL20 | Открытый стек Ethernet TSN (Time-Sensitive Networking) (TRL 4–6) | Отсутствие аппаратной поддержки PROFINET IRT (необходимы чипы Siemens ERTEC200/400). |
...
* **Программный слой:** Задействована лицензированная система [CODESYS V3 Runtime](../lib-1/CODESYS_Dependency.md#runtime-engine) с модифицированными стеками драйверов.
* **Аппаратные особенности:** Нативная аппаратная поддержка [EtherCAT Distributed Clocks (DC)](../lib-1/EtherCAT_Protocol.md#architecture), обеспечивающая синхронизацию сервоосей с точностью до 100 нс (TRL 9).
...
{% hint style="warning" %}
**Риск блокирования средообразующего ПО**
...
1. **[Beremiz Framework](../lib-1/CODESYS_Dependency.md#beremiz-framework) (TRL 5–6):** Открытая среда автоматизации, использующая компилятор `MatIEC` для трансформации стандартизированного кода (ST, LD, FBD) в ANSI C с последующей сборкой бинарных модулей через GCC/Clang под целевую ОСРВ.
2. **[Eclipse 4diac](../lib-1/IEC_61499_Standard.md#distributed-runtime) (TRL 4–5):** Реализация стандарта IEC 61499, основанная на событийно-ориентированной архитектуре (Event-driven execution). Позволяет распределять вычисления между независимыми ПЛК без общего координатора.
...
Применение концепций, описанных в [Суверенных фундаментальных LLM-моделях](../01_Frontier_TRL1-6/12_AI_Neuromorphic_Systems.md#суверенные-фундаментальные-llm-модели), позволяет разворачивать локальные (On-Premise) интеллектуальные агенты проверки безопасного кода:
...
| **Нерегулируемые закладки в кремнии КНР** | Средняя | Катастрофический | Сетевая изоляция (Air-Gapping), внедрение протоколов взаимной аутентификации на базе ГОСТ/SM4, применение [Zero-Trust ICS](../lib-1/Cybersecurity_ICS.md#zero-trust). |
...
{% hint style="info" %}
**Вопрос 1: Допустимо ли принятие предложения подрядчика в текущем виде?**
...
{% hint style="info" %}
**Вопрос 2: Какой архитектурный стек является технически корректным для данного задачи?**
...
1. **Дискретные процессы (Машиностроение, Сборка, TRL 8–9):** Допустима прямая миграция на китайские связки Inovance/Delta + EtherCAT. Вектор развития — постепенный перевод среды выполнения с CODESYS на [Beremiz/OpenPLC](../lib-1/CODESYS_Dependency.md#beremiz-framework).
...
3. **Фундаментальная независимость (TRL 1–6):** Стратегический приоритет — разработка отечественных/открытых процессорных ядер [RISC-V Lockstep](../lib-1/RISC_V_Industrial_Cores.md#lockstep-architecture), верифицированных микроядерных ОСРВ (класса `seL4` / `RT-Thread Safety`) и компиляторов распределенного стандарта [IEC 61499](../lib-1/IEC_61499_Standard.md#distributed-runtime).

---
*Ассоциированные связи:*
* [12_AI_Neuromorphic_Systems](../01_Frontier_TRL1-6/12_AI_Neuromorphic_Systems.md#суверенные-фундаментальные-llm-модели) — Интеллектуальный формальный аудит и авто-генерация безопасного кода ПЛК.
* [EtherCAT_Protocol](../lib-1/EtherCAT_Protocol.md#architecture) — Детерминированные протоколы передачи данных промышленного уровня.
* [CODESYS_Dependency](../lib-1/CODESYS_Dependency.md#runtime-engine) — Анализ рисков и архитектура замещения софтверного движка CODESYS.
* [RISC_V_Industrial_Cores](../lib-1/RISC_V_Industrial_Cores.md#lockstep-architecture) — Проектирование отказоустойчивых процессорных ядер TRL 1–6.
* [IEC_61499_Standard](../lib-1/IEC_61499_Standard.md#distributed-runtime) — Распределенные событийно-ориентированные архитектуры управления.
...
{% hint style="info" %}
****Введение в проблематику TRL 7–9****
...
На этом этапе критически важно обеспечить паритет интересов сторон, не допустив неконтролируемой утечки критических технологий. Базовые принципы этого баланса заложены в [Модели паритета технологического суверенитета](../00_Strategy/RU_CN_Sovereignty_Matrix.md#модель-паритета). Без прочного фундамента, заложенного на этапах фундаментальных исследований [TRL 1–3](../lib-1/TRL_1_3_Basic_Research.md#фундаментальные-заделы) и лабораторного прототипирования [TRL 4–6](../lib-1/TRL_4_6_Applied_R%26D.md#лабораторное-прототипирование), развертывание масштабируемых систем TRL 7–9 невозможно.
{% endhint %}
...
3. **Аудит чистоты IP:** Перед демонстрацией прототипа в КНР необходимо провести сквозной аудит всей цепочки создания технологии начиная с [TRL 1–3](../lib-1/TRL_1_3_Basic_Research.md#патентная-чистота), чтобы исключить наличие сторонних (в т.ч. западных) лицензий в составе ПО или аппаратной части.
...
* **Сегментированные патентные пулы (Segmented Patent Pools):** Права на базовую технологию (*Background IP*), разработанную на этапах [TRL 4–6](../lib-1/TRL_4_6_Applied_R%26D.md#разработка-кд), жестко закрепляются за российской стороной. Совместно создаваемая интеллектуальная собственность (*Foreground IP*) разграничивается по географическому и отраслевому признакам (*Field of Use*).
* **Асимметричное кросс-лицензирование:** РФ предоставляет лицензии на использование технологий TRL 7–8 исключительно для внутреннего рынка КНР без права реэкспорта. В обмен запрашиваются кросс-лицензии на китайские технологии массового производства (компонентная база, прецизионное литье, аддитивные технологии), соответствующие TRL 9, для использования в РФ и ЕАЭС. Подробнее о методологии защиты см. в [Инструкции по оборонительному патентованию](../lib-1/IP_Protection_Methodology.md#оборонительное-патентование).
...
{% hint style="warning" %}
****Важное правило оборонительного патентования****
...
Крупные китайские банки (ICBC, Bank of China, CCB) ограничивают операции с РФ из-за страха потери доступа к долларовой системе (вторичные санкции). Решение проблемы на уровне TRL 9 требует создания изолированной финансовой инфраструктуры, детально описанной в [Архитектуре финансовых шлюзов](../lib-1/Sanctions_Resilient_Infrastructure.md#финансовые-шлюзы).
...
{% hint style="info" %}
****Концепция «Черного ящика» (Black Box)****
...
{% hint style="info" %}
****Кейс-тест: Вывод совместной разработки на рынок КНР****
...
* **Вариант В:** Применить модель [технологического паритета](../00_Strategy/RU_CN_Sovereignty_Matrix.md#модель-паритета):
